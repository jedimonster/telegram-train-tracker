...
def get_train_times(departure_station, arrival_station, day_num=None):
    logger.debug(f"Getting train times from {departure_station} to {arrival_station} for day_num: {day_num}")
    ...
    train_times = [(travel['departureTime'], travel['arrivalTime'], len(travel['trains']) - 1) for travel in res['result']['travels']]
    logger.debug(f"Found {len(train_times)} trains for route {departure_station} to {arrival_station}")
    return train_times
...
def get_delay_from_api(from_station, to_station, hour) -> TrainTimes:
    ...
    for travel in timetable['result']['travels']:
        ...
        if train_position is None:
            logger.info('No info for train departing at {departure}, it is probably on time'.format(departure=scheduled_departure))
